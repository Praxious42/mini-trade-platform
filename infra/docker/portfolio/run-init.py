...
PGHOST = os.getenv("PGHOST", "mintrade-portfolio")
PGUSER = os.getenv("PGUSER", "postgres")
PGDATABASE = os.getenv("PGDATABASE", "mintrade-portfolio")
PGPASSWORD = os.getenv("PGPASSWORD", "password")
SCRIPTS_DIR = os.getenv("SCRIPTS_DIR", "/scripts")


def can_connect_with_psql(env):
    try:
...
def main():
    ...
    while not can_connect_with_psql(env):
        time.sleep(1)
